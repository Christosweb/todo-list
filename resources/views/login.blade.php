...
<body class="">
  ...
  @csrf
    ...
    <div class="form-floating mb-3">
      <input type="email" class="form-control" name="email" id="email" value="{{ $errors->any() ? old('email') : session('email')}}" placeholder="email">
      <label for="floatingInput">Email address</label>
      <small class="text-danger">{{$errors->first('email')}}</small>
    </div>
    <div class="form-floating ">
      <input type="password" class="form-control" id="Password" name="password"  value="{{ $errors->any() ? old('password') : session('password')}}" placeholder="Password">
