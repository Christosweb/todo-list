      ...
      <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse" >
        <!-- < class="position-sticky pt-3 sidebar-sticky"> -->
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link  {{request()->is('complete') ? 'link-dark':'link-primary'}}" href="{{ route('completed_tasks.complete') }}">
          ...
          <div class="card bg-info mt-3">
            <div class="card-body">
                ...
                <input class="form-check-input me-2"
                     ...
                     name="checkbox"
                     id="checkbox"
                     value="{{$completedTask->todo}}"
                     {{'checked'}}
                        >
                    <p class="me-auto text-capitalize">{{$completedTask->todo}}</p>
                    <button class="btn btn-warning me-2 edit" ><a class="link-dark text-decoration-none" href="{{route('edit_tasks.getEdit', $completedTask->id)}}">Edit</a></button>
                    <button class="btn btn-danger delete" data-delete="{{route('delete', $completedTask->id)}}">Delete</button>
                </div>
            </div>
          </div>
          @endforeach
